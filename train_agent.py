...
def train_agent(agent,
    ...
    while timestep < total_timesteps:
        ...
        while not done:
            ...
            next_obs, reward, done, _ = env.step(action)
            aloss, closs = agent.update(curr_obs, action, reward, next_obs, done, timestep)

            aloss_record.append(aloss)
            closs_record.append(closs)
    ...
    np.save("./actor_loss_normal3", np.array(aloss_record))
    np.save("./critic_loss_normal3", np.array(closs_record))
    np.save("./training_reward_normal3", np.array(reward_record))
    np.save("./acc_reward_normal3", np.array(array_of_mean_acc_rewards))
